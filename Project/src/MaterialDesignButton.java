...
public class MaterialDesignButton extends Button {
	...
	private Duration rippleDuration = Duration.millis(300);
	private double lastRippleHeight = 0;
	private double lastRippleWidth = 0;
	private Color rippleColor = Color.LIGHTSTEELBLUE;
	...
	private void createRippleEffect() {
		...
		parallelTransition.getChildren().addAll(scaleRippleTimeline,
				fadeTransition);
		...
		this.addEventHandler(MouseEvent.MOUSE_PRESSED,
				event -> {
					...
					// Recalculate ripple size if size of button from last time
					// was changed
				if (getWidth() != lastRippleWidth
						|| getHeight() != lastRippleHeight) {
					...
					try {
						rippleClip
								.setArcHeight(this.getBackground().getFills()
										.get(0).getRadii()
										.getTopLeftHorizontalRadius());
						rippleClip
								.setArcWidth(this.getBackground().getFills()
										.get(0).getRadii()
										.getTopLeftHorizontalRadius());
						circleRipple.setClip(rippleClip);
					} catch (Exception e) {
					}
					// Getting 45% of longest button's length, because we want
					// edge of ripple effect always visible
					double circleRippleRadius = Math.max(getHeight(),
							getWidth()) * 0.45;
					final KeyValue keyValue = new KeyValue(circleRipple
							.radiusProperty(), circleRippleRadius,
							Interpolator.EASE_OUT);
					final KeyFrame keyFrame = new KeyFrame(rippleDuration,
							keyValue);
